...
import java.security.SecureRandom;
	...
	public class wierdgame3 {														//class declaration

		static SecureRandom randomNumber = new SecureRandom();						//build scanner and rng
		...
		public static void combatCalculator(){
		...
		action = input.nextInt();																        	//declare variable for user input (as number) and aquire value from scanner object																
					...
					if (action == 1) {																										//if plyr choose option 1 (check with equality operator)
						System.out.printf("%n***%nThou striketh yon %s with a Foam Sword for %d (Why??)%n***%n", MonsterName, AP);				//print attack text:
							MonstHlth = MonstHlth - AP;																						//Calculate damage & update monster health using subtraction
									}						
					...
					System.out.printf("%n~~~You run, you run so far away~~~%n");																			//print retreat message:					
						...
						HP = HP - MonstAtk;																																	//Plyr loses health equivalent to monster health
														...
														/*if the monster dies*/
				if (MonstHlth <=0)  {
					System.out.printf("%nOH MY GOD WHAT HAVE YOU DONE TO THAT %d?!", MonsterName);					//print "victory message
									}
